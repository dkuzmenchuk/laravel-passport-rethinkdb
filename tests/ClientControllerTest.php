...
class ClientControllerTest extends PHPUnit_Framework_TestCase
{
    ...
    public function test_all_the_clients_for_the_current_user_can_be_retrieved()
    {
        ...
        $controller = new dkuzmenchuk\PassportRethinkdb\Http\Controllers\ClientController(
    ...
    public function test_clients_can_be_stored()
    {
        ...
        $clients->shouldReceive('create')->once()->with(1, 'client name', 'http://localhost')->andReturn($client = new dkuzmenchuk\PassportRethinkdb\Client);
    ...
    public function test_clients_can_be_updated()
    {
        ...
        $request->setUserResolver(function () {
            ...
            $user->clients->shouldReceive('find')->with(1)->andReturn(
                $client = Mockery::mock('dkuzmenchuk\PassportRethinkdb\Client')
            );
        ...
        $clients->shouldReceive('update')->once()->with(
            Mockery::type('dkuzmenchuk\PassportRethinkdb\Client'), 'client name', 'http://localhost'
        )->andReturn('response');
    ...
    public function test_clients_can_be_deleted()
    {
        ...
        $clients->shouldReceive('delete')->once()->with(
            Mockery::type('dkuzmenchuk\PassportRethinkdb\Client')
        )->andReturn('response');
